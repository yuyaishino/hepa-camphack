...
var $headerflag = false;       //headerflag
var $headerList;       //headerflag
...
function __construct($orientation='P',$unit='mm',$format='A4')
{
	...
	if(is_string($format))
	{
		...
		elseif($format=='b4')
			$format=array(728.50,1031.81);
		elseif($format=='a4')
			$format=array(595.28,841.89);
		elseif($format=='b5')
			$format=array(515.91,728.50);
		elseif($format=='a5')
		...
		elseif($format=='10x5inch')
			$format=array(360.00,720.00);
		elseif($format=='11x5inch')
			$format=array(360.00,792.00);
		else
...
function setHeaderflag($headerflag){
	$this->headerflag = $headerflag;
}
function setHeaderList($headerList){
...
{
	//Title of document
	//$this->title=$title;  帳票のタブ名は、「PDF」に統一することで決定
	$this->title="PDF";
}
...
{
	//Subject of document
	//$this->subject=$subject;  帳票のタブ名は、「PDF」に統一することで決定
	$this->subject="PDF";
}
...
function Header()
{
	...
	if($headerflag){
		...
		if($pageno >= 2){
			...
			foreach ($headerList AS $headerData){
				$inh = $headerData['height'];
				$this->SetFillColor($headerData['fillColor'][0],$headerData['fillColor'][1],$headerData['fillColor'][2]);
				$this->SetFont(GOTHIC, '' ,$headerData['font']);
				$printStr =  str_jp_utf8_encoding($headerData['name']);
				$this->Cell($headerData['width'], $headerData['height'],$printStr, 1, 0, $headerData['align'], 1);
			}
			$this->ln($inh);
		}
	}
