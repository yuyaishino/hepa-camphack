...
require('myfpdf.php');            // Original Class
...
$MBCMAP['SJISV']   = array ('CMap'=>'90msp-RKSJ-V'  ,'Ordering'=>'Japan1','Supplement'=>2);
...
function TopCell($w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = 0, $link = '')
{
	if(!is_string($txt)){
		$txt = "".$txt;
	}
	...
	if ($this->y + $h > $this->PageBreakTrigger
			...
			if ($txt != '') {
				...
				$s      .= sprintf('BT %.2f %.2f Td (%s) Tj ET', ($this->x + $dx) * $k, ($this->h - ($this->y + $this->FontSize)) * $k, $txt);
				$txt = stripslashes($txt);
				if ($this->underline) {
					$s  .= ' ' . $this->_dounderline($this->x+$dx, $this->y + .5 * $h + .3 * $this->FontSize, $txt);
				}
...
function LineCell($w, $h = 0, $border = 0, $ln = 0){
	...
	if ($border == 1) {
		$op = 'S';
		$s      = sprintf('%.2f %.2f %.2f %.2f re %s ', $x* $k, ($this->h - $y) * $k, $w * $k, -$h * $k, $op);
	}
	if (is_string($border)) {
		if (strpos(' ' . $border, 'L')) {
			$s .= sprintf('%.2f %.2f m %.2f %.2f l S ', $x * $k, ($this->h - $y) * $k, $x * $k, ($this->h - ($y+$h)) * $k);
	...
	$s .= sprintf('%.2F %.2F m %.2F %.2F l S',$x * $k, ($this->h - ($y+$h))* $k, ($x + $w)* $k, ($this->h - $y)* $k);
